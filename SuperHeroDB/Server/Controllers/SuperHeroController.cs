...
namespace SuperHeroDB.Server.Controllers
{
    ...
    public class SuperHeroController : ControllerBase
    {
        ...
        {
            return _context.SuperHeroes.Include(sh => sh.Comic).ToListAsync();
        }

        [HttpGet("{id}")]
        public async Task<IActionResult> GetSingleSuperHero(int id)
        {
            var hero = await _context.SuperHeroes.Include(sh => sh.Comic).FirstOrDefaultAsync(h => h.Id == id);
            if (hero == null)
        ...
        {
            var dbHero = await _context.SuperHeroes.Include(sh => sh.Comic).FirstOrDefaultAsync(h => h.Id == id);
            if (dbHero == null)
                return NotFound("Super Hero wasn't found.");
